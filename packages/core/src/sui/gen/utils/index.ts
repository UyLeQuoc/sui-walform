  ...
  if ('u8' in parsedTag) {
    ...
    if (pkg === MOVE_STDLIB_ADDRESS) {
      ...
      if (structTag.module === 'option' && structTag.name === 'Option') {
        // Manual non-null patch (re-apply if codegen overwrites): guard
        // `typeParams[0]` access for noUncheckedIndexedAccess strict mode.
...
export function normalizeMoveArguments(
  ...
  for (const [i, argType] of argTypes.entries()) {
    ...
    // Manual non-null patch (re-apply if codegen overwrites): handle the
    // `undefined` case under noUncheckedIndexedAccess.
    const bcsType = type === null || type === undefined ? null : getPureBcsSchema(type);
...
> extends BcsStruct<T, Name> {
  ...
  > {
    ...
    // Manual non-null patch (re-apply if codegen overwrites): destructure
    // position [0] is `T | undefined` under noUncheckedIndexedAccess.
    if (!res) {
