...
export function checkRedirect(object: SuiObjectResponse): string | null {
  ...
  if (object.data && object.data.display) {
    ...
    if (display.data && display.data['walrus site address']) {
      // Sui SDK now types Display.data values as `unknown`. Narrow to
      // string before returning to satisfy the function signature.
      const addr = display.data['walrus site address'];
