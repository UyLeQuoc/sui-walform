...
export class WalrusSitesRouter {
  ...
  public matchPathToRoute(
    ...
    logger.info('Attempting to match the provided `path` with the routes in the Routes object', {
      path,
    ...
    const match = filtered.reduce((a, b) => (a[0].length >= b[0].length ? a : b), filtered[0])[1];
