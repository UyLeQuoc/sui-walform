...
describe('getRoutesAndRedirects', () => {
  ...
  test('parses routes and redirects from BCS data', async () => {
    ...
    const spy = vi.spyOn(rpcSelector, 'multiGetObjects');
    spy.mockResolvedValue([makeBcsObjectResponse(routesBcs), makeBcsObjectResponse(redirectsBcs)]);
...
describe('matchPathToRedirect', () => {
  ...
  test('preserves different status codes', () => {
    ...
    expect(wsRouter.matchPathToRedirect('/docs/guide', redirectsExample)?.status_code).toBe(307);
    expect(wsRouter.matchPathToRedirect('/blog/old-x', redirectsExample)?.status_code).toBe(308);
  });
});
