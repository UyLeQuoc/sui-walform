...
describe('streamInflate', () => {
  ...
  test('throws error for data exceeding maximum size', () => {
    ...
    expect(() => streamInflate(largeData, false, smallMaxSize)).toThrow(
      'maximumFileSize should be greater than 65536! Current 1000.',
    );
  });

  test('throws error for invalid maximum file size', () => {
    const tooSmallMaxSize = 1000;
    expect(() => streamInflate(encoded, false, tooSmallMaxSize)).toThrow(
      'maximumFileSize should be greater than',
    );
  });
