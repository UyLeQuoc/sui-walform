...
describe('blobAggregatorEndpoint', () => {
  ...
  it('URL-encodes the blob_id', () => {
    ...
    expect(url.toString()).toBe(`https://agg.example.com/v1/blobs/${encodeURIComponent(blobId)}`);
  });
});
...
describe('quiltAggregatorEndpoint', () => {
  it('builds the correct URL without trailing slash on base', () => {
    ...
    expect(url).toBeInstanceOf(URL);
    expect(url.toString()).toBe('https://agg.example.com/v1/blobs/by-quilt-patch-id/patch-001');
  });
